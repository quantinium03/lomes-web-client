...
const ScrollableRow = ({ title, data, linkPrefix , titlePage}) => {
  ...
  return (
    <div className="mb-8 relative">
      <h2 className="text-2xl font-bold text-white mb-4"><Link to={titlePage}>{title}</Link></h2>
      <div className="relative group">
        ...
        >
          {data.map((media) => (
            <Link
              key={media.id || media.tvShowId || media.animeId}
